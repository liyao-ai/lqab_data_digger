...
class Search_base(object):
    ...
    def show_page(self,sql_last_p="",page_p=1):
        ...
        list_t = []
        str_t = ""
        sql_last_p = secret_lqab(sql_last_p,key_p=config.dic_config["secret_key"],salt_p = config.dic_config["secret_salt"],secret_if="no")
        ...
        sql_last_p = sql_last_p = secret_lqab(sql_last_p,key_p=config.dic_config["secret_key"],salt_p = config.dic_config["secret_salt"],secret_if="yes")
        ...
        for row in rows:
            ...
            if (res_t > 0):
                ...
                job_p = rows_t[0][6]
                job_p = job_p.strip()
                job_p = secret_lqab(job_p,key_p=config.dic_config["secret_key"],salt_p = config.dic_config["secret_salt"],secret_if="yes")
                ...
                kwd_p = secret_lqab(kwd_p,key_p=config.dic_config["secret_key"],salt_p = config.dic_config["secret_salt"],secret_if="yes")
    ...
    def link_more(self,dic_p={}):
        ...
        sql_last_p = secret_lqab(sql_last_p,key_p=config.dic_config["secret_key"],salt_p = config.dic_config["secret_salt"],secret_if="yes")
