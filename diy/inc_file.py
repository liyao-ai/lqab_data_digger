    ...
    def add(self,path_p="",type="dir"):
        ...
        try:
            if type == "dir":
                ...
                token = ""
                f = open(path_p,'w',encoding="utf-8")
                f.write(token)
                f.close()
...
class File_file(File_floder):
    ...
    # 打开编辑源码
    def open_source(self,path,file):
    ...
    # 保存源码
    def save_source(self,path,file,all_the_text):
        file_object = open(path + "//" + file, 'w')
    ...
    # 文件追加写入
    def write_add(self,path_p="",renew_if=0,content_p=""):
        if (renew_if == 1):
            self.remove(path=path_p,type="file") # 删除旧文件
        print("追加路径",path_p, "追加内容",content_p) # 调试用
        with open(path_p, 'a+') as f:
            f.write(content_p)
