...
class Feature_base(object):
    ...
    # min-max(极值)归一
    def dim_get_max_min(self,row_p=(),which_p=1,rows_max=(),rows_min=(),numb_dim=16):
    ...
    # Z-score(标准化)归一
    def dim_get_stand(self,row_p=(),which_p=2,rows_avg=(),rows_std=(),numb_dim=16):
    ...
    # 数据库批处理无量纲化
    def dim_db_make(self,name_model_p="x",row_feature_p=16,limit_p=0,which_p=2):
    
        txt = "数据库批处理无量纲化"
        dic_d = {}
        name_table_p = "lr_feature_train_" + name_model_p
        sql = "select "
        ...
        # 调用特征项
            ...
            # 生成特征分类器参数文件
            f = open(config.dic_config["path_f"] + "feature_min_" + name_model_p + ".txt", "w")
            ...
            #print (rows_max) #调试用
            # 生成特征分类器参数文件
            f = open(config.dic_config["path_f"] + "feature_max_" + name_model_p + ".txt", "w")
        ...
        if (which_p == 2):
            ...
            # 生成特征分类器参数文件
            f = open(config.dic_config["path_f"] + "feature_avg_" + name_model_p + ".txt", "w")
            ...
            if (res_std < 1):
                return "特征库无内容或数据库读取失败"
            # 生成特征分类器参数文件
            f = open(config.dic_config["path_f"] + "feature_std_" + name_model_p + ".txt", "w")
    ...
    # 数据库特征批处理生成
    def value_db_make(self,name_model_p="x",row_feature_p=16,limit_p=0,dim_p=16):
...
class Train_dae(Dae_base,Feature_base):
    ...
    # 由数据库生成训练文件
    def lr_csv_train(self,path_out_p="./data/lr/train_lr_",which_p=1,name_model_p="x",row_feature_p=16,dim_p=2):
        
        name_table_train = "lr_feature_train_" + name_model_p
        name_table_test = "lr_feature_test_" + name_model_p
        txt = ""
        txt_csv = ""
        sql = "select "
        j = 1
        ...
        for i in range(0,row_feature_p):
            sql += "f" + str(i+1) +","
        sql += "what from " + name_table_train
        res, rows = rs_basedata_mysql.read_sql(sql)
        print (sql) # 调试用
        if (res < 1):
        ...
        while j<=dim_p:
            ...
            # 生成结果文件
            f = open(path_out_p + str(j) + "_" + name_model_p + ".csv", "w")
            f.write(txt_csv)
            f.close()
            
            print (str(j) + " " + path_out_p + str(j) + "_" + name_model_p +  ".csv 文件已生成") #调试用
            
            j +=1
            
        # 生成测试集数据文件
        sql = "select question,what from " + name_table_test
        res, rows = rs_basedata_mysql.read_sql(sql)
        if (res < 1):
            return "测试集数据库为空或数据库读取错误"
        
        # 逐行写入测试文件
        f = open(path_out_p + "test_" + name_model_p + ".csv", "a+")
        for row in rows:
            try:
        ...
        f.close()
        print ("\n")
        print (path_out_p + "test_" + name_model_p + ".csv 文件已生成") #调试用
    ...
    # logistic回归
    def train_lr(self,path_input_p="../data/lr/train_lr_x.csv",path_out_p="../data/lr/result_lr_x.csv",steps_p=64,test_p=0):
        ...
        for row in rows:
            ...
            if (dic_t):
                ...
                for x in dic_t:
                    ...
                    if (res_t > 0 and x != 'numb_key_lqab'):
...
class Test_dae(Dae_base):
    ...
    def classifier_lr(self,path_input_p="../data/dae/result_lr_0.csv",numb_fea_p=2,dim_p=2):
        ...
        while j<=dim_p:
            porb = 0.0
            weights = mat(self.weights2matrix("../data/dae/result_lr_" + str(j) + "_" + name_model_p +  ".csv",delimiter=","))
...
class Feature_dae(Train_dae):
    ...
    # logistic回归分类器
    def what_class_lr(self,list_last=[],path_dae_p="./data/lr/",dim_p=7,test_p=0,name_model_p="x"):
    ...
    # 问题分类处理
    def question_cf(self,q_p="",numb_fea_p=16,dim_p=7,dic_min={},dic_max={},path_dae_p="./data/dae/",path_dic_p=config.dic_config["path_dic"]  +  ""):
        ...
        if (q_p != ""):
            ...
            what_is = self.what_class_lr(list_last=list_last,path_dae_p=path_dae_p,dim_p=dim_p,test_p=0)
    ...
    def divine_q_db(self,numb_fea_p=16,dim_p=2,limit_p=0):
        ...
        for row in rows:
            ...
            what_is = self.question_cf(q_p=row[1],numb_fea_p=numb_fea_p,dim_p=dim_p,dic_min=dic_min,dic_max=dic_max)
