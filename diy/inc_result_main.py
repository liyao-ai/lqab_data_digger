...
class Qa(Result_base):
    ...
    def __init__(self):
        ...
        self.list_feeling = ["嘛","吗","呢","哦","么","麽","啊","呀","哪","吧","啦","喽","谁"]
        self.list_query = ["什么","难道","行不行","究竟","怎的","怎样","怎么","如何","好不好","多长","多差","多少","多好","多坏","多久","有何","几斤"]

    # 问句识别决策树法
    def question_if(self,str_p="",list_feeling=[],list_query=[]):
        
        if (list_feeling == []):
            list_feeling = self.list_feeling
        if (list_query == []):
            list_query = self.list_feeling
        result_p = 0
    ...
    def vector_preprocess(self,tf_idf_if_p=0,t_name_p="qa",conn_data="",mark_p="v5",numb_for_p=0,renew_if_p=1,forward_if_p=0):
        ...
        while (j <= numb_for_p):
            ...
            for row in rows_t:
                ...
                #print ("分词方式",way_p) # 调试用
                # 主处理sql语句
                sql = ""
                if (row[1].strip() != "n/a" and tf_idf_if_p != 2 ):
                ...
                if (row[3].strip() != "n/a" and tf_idf_if_p != 2):
